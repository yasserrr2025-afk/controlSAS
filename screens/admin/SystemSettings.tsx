...
const AdminSystemSettings: React.FC<Props> = ({ systemConfig, setSystemConfig, resetFunctions, onAlert }) => {
  ...
  const buildTenantUrl = (params: Record<string, string> = {}) => {
  ...
  const schoolLoginUrl = buildTenantUrl();
  const studentInquiryUrl = buildTenantUrl({ student_inquiry: '1' });
  const tv2Url = buildTenantUrl({ tv2: '1' });
  ...
  return (
    <div className="space-y-10 animate-slide-up text-right pb-20">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="bg-white p-8 rounded-[2.5rem] shadow-md border border-slate-100 space-y-5">
          <div className="flex items-center gap-4">
            <div className="p-4 bg-blue-50 text-blue-600 rounded-[1.5rem]"><ShieldCheck size={30} /></div>
            <div>
              <h3 className="text-2xl font-black text-slate-900">رابط دخول المدرسة</h3>
              <p className="text-xs font-bold text-slate-400 mt-1">رابط خاص يحمل رمز المدرسة النشطة لعزل الدخول والبيانات.</p>
            </div>
          </div>
          <div className="bg-slate-50 border border-slate-100 rounded-2xl p-4 text-left dir-ltr overflow-x-auto">
            <code className="text-blue-700 font-mono text-sm whitespace-nowrap">{schoolLoginUrl}</code>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <button onClick={() => handleCopy(schoolLoginUrl, 'school-login')} className="bg-blue-600 text-white px-6 py-4 rounded-2xl font-black flex items-center justify-center gap-2">
              {copied === 'school-login' ? <Check size={20} /> : <Copy size={20} />} نسخ رابط الدخول
            </button>
            <button onClick={() => window.open(schoolLoginUrl, '_blank', 'noopener,noreferrer')} className="bg-slate-100 text-slate-700 px-6 py-4 rounded-2xl font-black flex items-center justify-center gap-2">
        ...
        <div className="bg-white p-8 rounded-[2.5rem] shadow-md border border-slate-100 space-y-5">
          <div className="flex items-center gap-4">
            <div className="p-4 bg-orange-50 text-orange-600 rounded-[1.5rem]"><MonitorPlay size={30} /></div>
            <div>
              <h3 className="text-2xl font-black text-slate-900">رابط استعلام الطلاب</h3>
              <p className="text-xs font-bold text-slate-400 mt-1">رابط عام للطلاب مرتبط برمز المدرسة الحالية.</p>
            </div>
          </div>
          <div className="bg-slate-50 border border-slate-100 rounded-2xl p-4 text-left dir-ltr overflow-x-auto">
            <code className="text-orange-700 font-mono text-sm whitespace-nowrap">{studentInquiryUrl}</code>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <button onClick={() => handleCopy(studentInquiryUrl, 'student-inquiry')} className="bg-orange-500 text-white px-6 py-4 rounded-2xl font-black flex items-center justify-center gap-2">
              {copied === 'student-inquiry' ? <Check size={20} /> : <Copy size={20} />} نسخ رابط الاستعلام
            </button>
            <button onClick={() => window.open(studentInquiryUrl, '_blank', 'noopener,noreferrer')} className="bg-slate-100 text-slate-700 px-6 py-4 rounded-2xl font-black flex items-center justify-center gap-2">
              <ExternalLink size={20} /> فتح الرابط
            </button>
          </div>
        </div>
